"""
Prometheus metrics for the parking occupancy detection service.
"""
from prometheus_client import Counter, Histogram, Gauge, Info

# === Info ===
SERVICE_INFO = Info("parking_ai_service", "Parking occupancy service information")
...
BACKEND_PUSH_TOTAL = Counter(
    "parking_ai_backend_push_total",
    "Total number of push attempts to backend service",
...
BACKEND_PUSH_DURATION = Histogram(
    "parking_ai_backend_push_duration_seconds",
    "Duration of push requests to backend service",
    ["parking_lot_id", "camera_id"],
...
def record_backend_push(parking_lot_id: str, camera_id: int, status: str, duration: float):
    BACKEND_PUSH_TOTAL.labels(parking_lot_id=parking_lot_id, camera_id=str(camera_id), status=status).inc()
    if status == "success":
        BACKEND_PUSH_DURATION.labels(parking_lot_id=parking_lot_id, camera_id=str(camera_id)).observe(duration)
